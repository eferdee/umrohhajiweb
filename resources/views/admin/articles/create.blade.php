        ...
        <form method="POST" action="{{ route('admin.articles.store') }}" enctype="multipart/form-data" class="space-y-5">
            ...
            <div>
                ...
                <textarea name="content" rows="8"
                          class="w-full border border-[var(--color-line)] rounded-lg px-4 py-2.5 text-sm focus:outline-none focus:ring-2 focus:ring-[var(--color-primary)]/30 @error('content') border-[var(--color-danger)] @enderror">{{ old('content') }}</textarea>
            ...
            <label class="flex items-center gap-2 text-sm">
                <input type="checkbox" name="is_published" value="1" class="rounded border-[var(--color-line)] text-[var(--color-primary)]">
                Terbitkan sekarang
            </label>
