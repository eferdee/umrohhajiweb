...
@section('content')
    ...
    <div class="bg-[var(--color-surface)] rounded-[var(--radius-card)] border border-[var(--color-line)] p-6 max-w-xl mt-4">
        ...
        <form method="POST" action="{{ route('admin.packages.schedules.update', [$package, $schedule]) }}" class="space-y-5">
            @csrf @method('PUT')

            <div>
                <label class="block text-sm font-medium mb-1.5">Kota Keberangkatan</label>
                <input type="text" name="departure_city" value="{{ old('departure_city', $schedule->departure_city) }}"
            ...
            <div class="grid grid-cols-2 gap-4">
                <div>
                    ...
                    <input type="date" name="departure_date" value="{{ old('departure_date', $schedule->departure_date->format('Y-m-d')) }}"
                ...
                <div>
                    ...
                    <input type="date" name="return_date" value="{{ old('return_date', $schedule->return_date->format('Y-m-d')) }}"
            ...
            <div class="grid grid-cols-2 gap-4">
                ...
                <div>
                    ...
                    <input type="number" name="quota" value="{{ old('quota', $schedule->quota) }}"
            ...
            <p class="text-xs text-[var(--color-ink-soft)]">Kursi tersisa saat ini: <strong>{{ $schedule->available_seat }}</strong> dari {{ $schedule->quota }}. Mengubah kuota akan menyesuaikan sisa kursi secara proporsional.</p>

            <label class="flex items-center gap-2 text-sm">
                <input type="checkbox" name="status" value="1" {{ old('status', $schedule->status) ? 'checked' : '' }} class="rounded border-[var(--color-line)] text-[var(--color-primary)]">
                Aktifkan jadwal ini
            </label>
